...
# =============================================================================
# YAML Parsing (uses yq — like jq but for YAML)
# =============================================================================
...
function yamlGet() {
  ...
  # Convert dot-separated path to yq path (e.g., "proxmox.ip" → ".proxmox.ip")
  local yq_path=".$key"

  yq -r "$yq_path // \"\"" "$file" 2>/dev/null
}
...
function generateTfvarsFromBootstrap() {
  ...
  for i in 0 1 2; do
    local idx=$((i % ${#CLUSTER_NODES[@]}))
    local target_node="${CLUSTER_NODES[$idx]}"
  ...
  # Build Kasm VM config — single node at BASE.20 by convention (matches
  # CLAUDE.md VMID 930 and the previous .120 default scheme on 10.1.50.0/24).
  local KASM_CONFIGS_HCL=""
  KASM_CONFIGS_HCL="  \"kasm01\" = { vm_id = 930, name = \"kasm01\", ip = \"${BASE_IP}.20\", cores = 4, memory = 8192, disk_size = \"100G\", vm_state = \"running\", target_node = \"${CLUSTER_NODES[0]}\", target_storage = \"${RUNTIME_STORAGE}\" }\n"
...
# Nomad VM configs — IPs derived from network.cidr, target_node round-robin
nomad_vm_configs = {
$(echo -e "$NOMAD_CONFIGS_HCL")}

# Kasm VM config — single node, IP derived from network.cidr
kasm_vm_configs = {
$(echo -e "$KASM_CONFIGS_HCL")}
