...
EOF
  ...
  for ip in $ALL_NOMAD_IPS; do
    ...
    # Update the Vault address in the WIF config (skeleton written by cloud-init)
    if ! sshRunAdmin "$VM_USER" "$ip" "sudo sed -i 's|address = .*|address = \"${VAULT_ADDR}\"|' /etc/nomad.d/vault.hcl"; then
      error "Failed to update Vault address on $hostname"
      return 1
    fi
