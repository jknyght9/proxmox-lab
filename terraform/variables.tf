...
variable "dns_use_dot" {
  type        = bool
  description = "Use DNS-over-TLS to Cloudflare/Quad9 as Pi-hole/Unbound's upstream. Set false on networks that block outbound 853/tcp — Unbound falls back to plain UDP/53 against bootstrap_dns instead."
  default     = true
}
...
# =============================================================================
# Nomad VM Configuration
# =============================================================================

# Generated by bootstrap from bootstrap.yml network.cidr — passed through
# to module.nomad so IPs match the deploy network instead of the
# vm-nomad/variables.tf defaults (which were lab-specific).
variable "nomad_vm_configs" {
  type = map(object({
    vm_id          = number
    name           = string
    ip             = string
